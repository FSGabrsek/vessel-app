...
const props = defineProps(['target', 'follows']);
...
async function follow_user() {
    await follow(auth.user.value._id, props.target._id)
    emit('reload')
}

async function unfollow_user() {
    await unfollow(auth.user.value._id, props.target._id)
    emit('reload')
}
</script>
...
<template>
    <div class="wrapper">
        <h4><RouterLink :to="{ name: 'watch', params: { userId: props.target._id } }">{{ target.username }}</RouterLink></h4>
        <div v-if="follows" class="heart" @click="unfollow_user">
        ...
        <div v-if="(!follows) && (auth.user.value._id !== props.target._id)" class="heart" @click="follow_user">
